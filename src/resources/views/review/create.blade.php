...
<form action="{{ isset($review) ? route('review.update', $review->id) : route('review.store') }}" method="post"
    ...
    <div>
        ...
        <div class="rating-stars" id="rating-stars">
            @for ($i = 1; $i <= 5; $i++) <span
                class="star {{ old('rating', $review->rating ?? 0) >= $i ? 'selected' : '' }}" data-value="{{ $i }}">
                ★</span>
                @endfor
        </div>
    ...
    <div>
        ...
        <textarea name="comment" id="comment"
            placeholder="レビューは400文字以内でご入力ください。">{{ old('comment', $review->comment ?? '') }}</textarea>
    </div>
    ...
    <div class="form__group">
        ...
        <div class="form__group-content">
            ...
            <div class="drag-drop-area" id="drag-drop-area">
                ここにファイルをドラッグ＆ドロップ<br>またはクリックして選択<br>JPEGまたはPNGファイルのみ可能
