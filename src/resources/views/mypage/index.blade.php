                    ...
                    <div class="reservation__contents">
                        <div class="reservation__header">
                            ...
                            <div class="reservation__menus">
                                <button title="QRを表示" class="icon-button qr-button"
                                    popovertarget="Modal{{ $reservation->id }}" popovertargetaction="show"></button>
                                <div popover id="Modal{{ $reservation->id }}">
                                    <div class="qr-code">
                                        {!! QrCode::size(150)->generate(url('/reservation/verify/' . $reservation->id)) !!}
                                    </div>
                                    <button popovertarget="Modal{{ $reservation->id }}"
                                        popovertargetaction="hidden">閉じる</button>
                                </div>
                                <form action="/reservation/edit/{{ $reservation->id }}" class="reservation__edit"
                                    method="get">
                                ...
                                <form action="/reservation" method="post">
                                    ...
                                    <button class="icon-button delete-button" type="submit"
                                        onclick="return confirm('予約を削除しますか？')" title="予定を削除"></button>
                                </form>
                            </div>
                        </div>
                        <h4>{{ $reservation->shop->shop_name }}</h4>
                        <p>Date:
                            {{ \Carbon\Carbon::parse($reservation->reserve_date)->locale('ja')->isoFormat('YYYY-MM-DD (dd)') }}
                        </p>
